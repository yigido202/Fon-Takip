...
def sepet_otomatik(kod):
    ...
    if pdf_icerik:
        try:
            ag, notu = _pdf_sepet_coz(pdf_icerik)
            if ag:
                return ag, etiket, f"{etiket}: OK — KAP haftalik, {notu}"
    ...
    for geri in range(3):
        ...
        try:
            ...
            if b"%PDF" not in r.content[:2048]:
                ...
                continue
            ag, notu = _pdf_sepet_coz(r.content)
            if ag:
                return ag, donem, f"{donem}: OK — aylik, {notu}"
