...
AYIKLAYICI_SURUM = 12   # parser her degistiginde artir -> onbellek yenilenir

def _fintables_sayfasi(kod, tani):
    """PDF yoksa fintables.com/fonlar/{kod} sayfasindaki pozisyonlari dener."""
    try:
        r = requests.get(f"https://fintables.com/fonlar/{kod}",
                         headers={"User-Agent": TARAYICI["User-Agent"],
                                  "Accept": "text/html"}, timeout=10)
        if r.status_code != 200:
            ...
            try:
                ...
                def gez(o):
                    if isinstance(o, dict):
                        kodlar = [v for v in o.values()
                                  if isinstance(v, str)
                                  and re.fullmatch(r"[A-Z]{4,6}", v)]
                        sayilar = [v for k, v in o.items()
                                   if isinstance(v, (int, float))
                                   and 0 < v <= 60
                                   and re.search(r"weight|ratio|oran|pct|"
                                                 r"percent", k, re.I)]
                        if kodlar and sayilar                                 and kodlar[0] not in _SEPET_KARA_LISTE:
                            ag[kodlar[0]] = ag.get(kodlar[0], 0)                                 + float(sayilar[0])
                        for v in o.values():
                            gez(v)
                ...
                pass
        if len(ag) < 3:          # kaba HTML taramasi
            ag = {}
            for mm in re.finditer(
                    r'"([A-Z]{4,6})"[^{}\[\]]{0,120}?(\d{1,2}\.\d{1,2})',
                    r.text):
                h, w = mm.group(1), float(mm.group(2))
                if h not in _SEPET_KARA_LISTE and 0 < w <= 60 and h not in ag:
                    ag[h] = w
        toplam = sum(ag.values())
        if len(ag) >= 3 and 15 <= toplam <= 110:
            return ag
        tani.append(f"Fintables sayfa: {len(ag)}h/%{toplam:.0f}")
    except Exception as e:
        tani.append(f"Fintables sayfa: {type(e).__name__}: {str(e)[:40]}")
    return None
