...
def yarim_saatlik_ozet(toplanan, sepetler, deg):
    """Her calismada gonderilen tam durum ozeti."""
    satirlar = [f"📊 *Yarim Saatlik Ozet* — {simdi_tr().strftime('%H:%M')}"]
    for kod in FONLAR:
        t = toplanan.get(kod)
        if t is None or len(t) < 2:
        ...
        # sepetten anlik ~tahmin (hisse fonlari)
        tah_s = ""
        sepet = sepetler.get(kod)
        if sepet and deg is not None and not deg.empty:
            ...
            if kapsam >= 20:
                tah = sum(son_deg.get(h, 0) * w for h, w in sepet.items()
                          if h in son_deg.index and son_deg[h] == son_deg[h]) / kapsam
        ...
        satirlar.append(
            ...
            f"  Tutar: g {kisa(tg)} · h {kisa(th)} TL")
    # Portfoy durumu
    portfoy = portfoy_oku()
    if portfoy:
        ...
        for kod, (adet, maliyet) in portfoy.items():
            ...
            if maliyet > 0:
                ...
                kz_s = f" (K/Z {kisa(kz)} TL)"
            p_satir.append(f"  {kod}: {kisa(deger, isaret=False)} TL{kz_s}")
        if p_satir:
            satirlar.append("💼 *Portfoyum*")
            satirlar.extend(p_satir)
            toplam_s = f"  Toplam: {kisa(toplam_d, isaret=False)} TL"
            if toplam_m > 0:
                kz_t = toplam_d - toplam_m
                toplam_s += (f" · K/Z {kisa(kz_t)} TL "
                             f"(%{kz_t/toplam_m*100:+.1f})".replace(".", ","))
            satirlar.append(toplam_s)
    else:
        satirlar.append("💼 Portfoy: PORTFOY_JSON secret'i tanimli degil")
    satirlar.append("_Yatirim tavsiyesi degildir._")
    return "\n".join(satirlar)
...
def alarm_kontrol():
    ...
    try:
        bildirim_gonder(yarim_saatlik_ozet(toplanan, sepetler, deg))
    except Exception as e:
