...
# (alim valoru, satim valoru) is gunu — fon izahnamelerinden; degisiklikleri
# kontrol edin. Para piyasasi fonlari genelde ayni gun (T+0) islenir.
VALOR = {"TLY": (1, 2), "THF": (1, 2), "DOH": (1, 2),
         "TP2": (0, 0), "PRY": (0, 0), "PNU": (0, 0)}
EMIR_KESIM_SAATI = 13.5  # TEFAS standart emir kesimi ~13:30 (kurucuya gore degisebilir)

def simdi_tr():
    """Sunucu UTC'de calisir; Turkiye saati = UTC+3."""
    return datetime.utcnow() + timedelta(hours=3)

def is_gunu_ekle(baslangic, n):
    """Hafta sonlarini atlayarak n is gunu ekler (resmi tatiller haric)."""
    g = baslangic
    while g.weekday() >= 5:  # hafta sonuna denk geldiyse ilk is gunune kay
        g += timedelta(days=1)
    for _ in range(n):
        g += timedelta(days=1)
        while g.weekday() >= 5:
...
GUN_ISIM = ["Pazartesi", "Sali", "Carsamba", "Persembe", "Cuma",
            "Cumartesi", "Pazar"]
# Valor (is gunu): alim = paylarin gecisi, satim = paranin gecisi.
# TLY degerleri fonun kendi sayfasindan; digerleri tipik degerlerdir,
# kesin bilgi icin fonun izahnamesine/yatirimci bilgi formuna bakin.
VALOR = {
    "TLY": {"alim": 1, "satim": 2},
    "THF": {"alim": 1, "satim": 2},
    "DOH": {"alim": 1, "satim": 2},
...
KESIM_SAATI = "13:30"  # TEFAS fonlarinda yaygin emir kesim saati

def is_gunu_ekle(tarih, n):
    """Hafta sonlarini atlayarak n is gunu ekler (resmi tatiller haric)."""
    while tarih.weekday() >= 5:  # emir gunu hafta sonuysa ilk is gunune kay
...
GUN_TR = ["Pazartesi", "Sali", "Carsamba", "Persembe", "Cuma",
          "Cumartesi", "Pazar"]

def tarih_tr(t):
    return f"{t.strftime('%d.%m.%Y')} {GUN_TR[t.weekday()]}"
FON_ACIKLAMA = {
...
with sekmeler[0]:
    ...
    v_islem = v2.radio("Islem", ["Alim", "Satim"], horizontal=True,
                       key="valor_islem")
    simdi = simdi_tr()
    alim_v, satim_v = VALOR.get(v_fon, (1, 2))
    n = alim_v if v_islem == "Alim" else satim_v
    # Emir bugun mu isleme girer?
    talimat = simdi.date()
    gec_kaldi = (simdi.hour + simdi.minute / 60) >= EMIR_KESIM_SAATI
    if talimat.weekday() >= 5 or gec_kaldi:
        talimat = is_gunu_ekle(talimat + timedelta(days=1), 0)
        neden = ("hafta sonu" if simdi.weekday() >= 5
                 else "emir kesim saati (13:30) gectigi icin")
        giris_notu = f"Su an verecegin emir {neden} **{GUN_ISIM[talimat.weekday()]} {talimat:%d.%m}** gunu isleme alinir."
    else:
        giris_notu = f"Su an verecegin emir bugun (**{GUN_ISIM[talimat.weekday()]} {talimat:%d.%m}**) isleme alinir."
    sonuc_gun = is_gunu_ekle(talimat, n)
    if v_islem == "Alim":
        if n == 0:
            sonuc = f"Paylar **ayni gun ({sonuc_gun:%d.%m})** hesabina gecer (T+0)."
        else:
            sonuc = (f"Paylar **{GUN_ISIM[sonuc_gun.weekday()]} {sonuc_gun:%d.%m}** "
                     f"hesabina gecer ve o tarihli fiyat uygulanir (T+{n}).")
    else:
        if n == 0:
            sonuc = f"Satis bedeli **ayni gun ({sonuc_gun:%d.%m})** hesabina gecer (T+0)."
        else:
            sonuc = (f"Satis bedeli **{GUN_ISIM[sonuc_gun.weekday()]} "
                     f"{sonuc_gun:%d.%m}** gunu hesabina gecer (T+{n}) — "
                     "dusus aninda 'hemen cikis' olmadigini unutma.")
    st.markdown(f"<div class='analiz-kutu'>{giris_notu}<br>{sonuc}<br>"
                f"<i>Valorler ({v_fon}: alim T+{alim_v}, satim T+{satim_v}) "
                "fon izahnamesinden alinmistir; resmi tatiller ve kurucu "
                "degisiklikleri icin araci kurumundan teyit et.</i></div>",
                unsafe_allow_html=True)
    ...
    # Emrin sayilacagi gun: hafta sonu veya 13:30 sonrasi -> sonraki is gunu
    ...
    n = VALOR[v_fon][0 if v_islem == "Alim" else 1]
