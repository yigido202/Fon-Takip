        ...
        try:
            ...
            with pdfplumber.open(io.BytesIO(r.content)) as pdf:
                for sayfa in pdf.pages:
                    ...
                    try:                      # tablo hucrelerini de satira cevir
                        for tablo in sayfa.extract_tables() or []:
                            for hucre in tablo:
                                satirlar.append(" ".join(str(h) for h in hucre if h))
                    except Exception:
                        pass
            agirliklar, gorulen = {}, set()
            for satir in satirlar:
                ...
                try:
                    ...
                    continue
                # Ayni hisse + ayni agirlik ikinci kez gelirse (metin + tablo) sayma
            ...
            if len(agirliklar) >= 3 and 25 <= toplam <= 110:
                return agirliklar, donem, f"{donem}: OK ({len(agirliklar)} hisse, toplam %{toplam:.0f})"
            tani.append(f"{donem}: PDF acildi ama {len(agirliklar)} hisse/%{toplam:.0f} "
                        f"(beklenen ≥3 hisse, %25-110) — {len(satirlar)} satir")
        except Exception as e:
...
def saglik_paneli():
    ...
    with st.expander(f"🩺 Kaynak sagligi — {'sorun yok' if not sorun else str(sorun) + ' kaynakta sorun'}",
                     expanded=bool(sorun)):
