...
FONLAR = ["TLY", "THF", "DOH", "DFI", "BSM", "GAL", "GTL",
          "TP2", "PRY", "PNU"]
...
SERBEST_FONLAR = ["TLY", "DOH", "DFI", "BSM", "GAL", "GTL"]              # yeni kurallara dogrudan tabi
...
/* Grafik ve tablolarin sag ust arac cubugu (tablo gorunumu/buyutme) kapali:
   yanlislikla acilip kapatilamayan veri tablosu sorununu bitirir */
...
with st.sidebar.form("portfoy_formu"):
    st.caption("Adet · alis fiyati · alis tutari — ucunden IKISINI gir, "
               "kalanini Kaydet'te ben hesaplarim.")
    _girisler, _maliyet_girisler, _tutar_girisler = {}, {}, {}
    for _kod in FONLAR:
        try:
        ...
        _girisler[_kod] = _c1.number_input(f"{_kod} adet", min_value=0.0,
                                           ...
                                           format="%.2f")
        _maliyet_girisler[_kod] = _c2.number_input(f"{_kod} alis fiyat",
                                                   min_value=0.0,
                                                   value=_mvars, step=0.1,
                                                   format="%.4f")
        _tutar_girisler[_kod] = _c3.number_input(f"{_kod} alis tutar",
                                                 min_value=0.0,
                                                 value=round(_vars * _mvars, 2),
...
with st.sidebar.expander("🔔 Alarm esiklerim (%)"):
    ...
    with st.form("esik_formu"):
        ...
        for _kod in FONLAR:
            ...
            try:
                ...
                _yv = 0.0
            e1, e2 = st.columns(2)
            _esikler[_kod] = e1.number_input(
                f"{_kod} dusus", min_value=-99.0, max_value=0.0,
                value=_ev, step=0.1, format="%.1f", key=f"esk_d_{_kod}")
            _esikler_yuk[_kod] = e2.number_input(
                f"{_kod} yukselis", min_value=0.0, max_value=99.0,
                value=_yv, step=0.1, format="%.1f", key=f"esk_y_{_kod}")
            _mad = st.session_state.get("karne_mad", {}).get(_kod)
            if _mad is not None:
                _oneri = -math.ceil((1.0 + 2 * _mad) * 2) / 2
                if _esikler[_kod] > _oneri + 1e-9:
                    st.caption(f"⚠ Tahmin sapmasi ±{_mad:.2f}; dusus esigi "
                               f"gurultuye takilabilir. Oneri: {_oneri:g}")
    ...
    st.caption("Dusus ornegi: -3,5 → gunluk %-3,5 ve altinda alarm. "
               "Yukselis ornegi: 2 → gunluk %+2 ve ustunde alarm (kar "
               "realizasyonu hatirlatici gibi). Degisiklikler bu cihazda "
               "saklanir; alarmlar gunde 1 kez gider.")
...
with sekmeler[0]:
    ...
    degisim_kolonlari = ["Yat. Δ (gun)", "Yat. Δ (hafta)", "Yat. Δ (ay)",
                         ...
                         "K/Z (TL)"]
    def _fon_tablo_ciz(df_k, baslik):
        if df_k.empty:
            return
        st.markdown(f"**{baslik}**")
