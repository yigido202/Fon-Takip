...
def donut_ciz(veri, baslik, yukseklik=230):
    """{ad: pay} sozlugunu daire grafik olarak cizer; eksileri altta listeler."""
    pozitif = {a: v for a, v in (veri or {}).items() if v > 0}
    negatif = {a: v for a, v in (veri or {}).items() if v <= 0}
    if not pozitif:
        return False
    st.markdown(f"**{baslik}**")
    d_df = pd.DataFrame({"Kalem": list(pozitif.keys()),
                         "Pay": [round(v, 2) for v in pozitif.values()]})
    grafik = (alt.Chart(d_df).mark_arc(innerRadius=50).encode(
        theta=alt.Theta("Pay:Q"),
        color=alt.Color("Kalem:N", scale=alt.Scale(scheme="tableau20"),
    ...
    for a, v in negatif.items():
        st.markdown(f"<span style='color:#c62828'>▼ {a}: %{v:.2f}</span>",
