...
# ----------------- TEFAS (cerezli oturum + yedek kaynak) -----------------
KAYNAKLAR = [
...
def fon_gecmisi(kod, gun=21):
    """Fonun gunluk fiyat/yatirimci/buyukluk gecmisi + kaynak adi."""
    bitis = datetime.now()
    baslangic = bitis - timedelta(days=gun)
    govde = {"fontip": "YAT", "sfontur": "", "fonkod": kod, "fongrup": "",
    ...
    for site, ad in KAYNAKLAR:
        try:
            ...
            # 1) Cerez almak icin sayfayi ziyaret et
            oturum.get(f"{site}/TarihselVeriler.aspx", timeout=12)
            # 2) Veriyi iste
            r = oturum.post(f"{site}/api/DB/BindHistoryInfo", data=govde,
                            headers={"Origin": site,
