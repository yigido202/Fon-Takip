...
ALARM_ESIK = {7: -3.5, 6: -2.5, 5: -2.5, 4: -1.5, 3: -1.5, 2: -0.1, 1: -0.1}
...
if WA_TEL and WA_KEY:
    ...
    for kod in FONLAR:
        b = FON_ANALIZ.get(kod, {})
        gunluk = b.get("gunluk")
        esik = ALARM_ESIK.get(RISK_YEDEK.get(kod, 5), -2.0)
        if gunluk is not None and gunluk <= esik \
                and f"{kod}-esik" not in gidenler[bugun_a]:
            alarmlar.append(f"🔴 *{kod}* gunluk %{gunluk:+.2f} — risk esigi "
                            f"(%{esik}) asildi.")
...
---
Esikler risk seviyesine goredir: 7/7 fonlarda %-3,5 | 5-6/7'de %-2,5 |
para fonlarinda eksi gun aninda bildirilir.
