...
def sepet_otomatik(kod):
    """(agirliklar, donem, tani) — tani: her denemenin neden basarisiz oldugu."""
    tani = []
    try:
        import pdfplumber
    except Exception as e:
    ...
    for geri in range(3):
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                continue
            if b"%PDF" not in r.content[:2048]:   # PDF basligi ilk 2 KB icinde olmali
                tani.append(f"{donem}: PDF degil ({r.headers.get('Content-Type','?')}, "
            ...
            with pdfplumber.open(io.BytesIO(r.content)) as pdf:
                for sayfa in pdf.pages:
                    ...
                    try:                      # tablo hucreleri sadece YEDEK
            ...
            def _sutun_bazli(tablolar):
                """Baslik satirindan FTD % sutununu bulup dogrudan okur."""
                ag = {}
            ...
            denemeler = [("metin", _ayikla(satirlar)),
                         ("FTD sutunu", _sutun_bazli(tablolar)),
                         ("tablo satiri", _ayikla(tablo_satirlari))]
            notlar = []
            for ad, agirliklar in denemeler:
                toplam = sum(agirliklar.values())
                if len(agirliklar) >= 3 and 25 <= toplam <= 110:
                    return agirliklar, donem, (f"{donem}: OK — {ad} yontemi "
                                               f"({len(agirliklar)} hisse, toplam %{toplam:.0f})")
                notlar.append(f"{ad} {len(agirliklar)} hisse/%{toplam:.0f}")
            tani.append(f"{donem}: PDF acildi ama gecerli sepet cikmadi [" + "; ".join(notlar)
                        + f"] — {len(satirlar)} satir, {len(tablolar)} tablo")
        except Exception as e:
            tani.append(f"{donem}: {type(e).__name__}: {str(e)[:60]}")
    return None, None, " | ".join(tani) if tani else "denenmedi"

SEPET_TANI = {}
SEPET_KAYNAK = {}
for _kod in list(SEPETLER):
    _oto, _donem, _tani = (sepet_otomatik(_kod) if oto_sepet else (None, None, "kapali"))
...
for _kod in EK_FONLAR:
    ...
    if oto_sepet and _kod not in MEVDUAT_BENZERI:
        _oto, _donem, _tani = sepet_otomatik(_kod)
        SEPET_TANI[_kod] = _tani
        if _oto:
            SEPETLER[_kod] = _oto
            SEPET_KAYNAK[_kod] = f"KAP {_donem} raporu (otomatik)"
...
def hisse_verileri(hisseler):
    ...
    if gelen >= max(3, int(len(hisseler) * 0.5)):
        ...
        return METRIK, DEG, ""
    # Yahoo dustu: son basarili hisse verisini goster
