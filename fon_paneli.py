...
for _kod in FONLAR:
    try:
        _varsayilan = float(st.query_params.get(_kod, "0"))
    except (TypeError, ValueError):
        _varsayilan = 0.0
    ADETLER[_kod] = st.sidebar.number_input(
        _kod, min_value=0.0, value=_varsayilan, step=1.0,
        format="%.2f", key=f"adet_{_kod}")
for _kod, _adet in ADETLER.items():
    if _adet > 0:
        st.query_params[_kod] = f"{_adet:g}"
    elif _kod in st.query_params:
        del st.query_params[_kod]
st.sidebar.caption("Adetler sitenin adresine islenir: bu sayfayi yer imine "
                   "eklersen adetlerin bir daha girmene gerek kalmadan gelir.")
