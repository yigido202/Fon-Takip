...
# --- Portfoyum: fon adetleri (URL'e yazilir, yer imiyle kalici olur) ---
st.sidebar.markdown("---")
st.sidebar.subheader("Portfoyum (adet)")

def _adet_degisti(kod):
    """Kullanici degeri degistirince URL'i guncelle (her cizimde degil)."""
    v = st.session_state.get(f"adet_{kod}", 0.0)
    if v and v > 0:
        st.query_params[kod] = f"{v:g}"
    elif kod in st.query_params:
        del st.query_params[kod]

for _kod in FONLAR:
    _anahtar = f"adet_{_kod}"
    if _anahtar not in st.session_state:  # ilk aciliste URL'den yukle
        try:
            st.session_state[_anahtar] = float(st.query_params.get(_kod, "0"))
        except (TypeError, ValueError):
            st.session_state[_anahtar] = 0.0
    st.sidebar.number_input(_kod, min_value=0.0, step=1.0, format="%.2f",
                            key=_anahtar, on_change=_adet_degisti,
                            args=(_kod,))
ADETLER = {k: float(st.session_state.get(f"adet_{k}", 0.0) or 0.0)
           for k in FONLAR}
st.sidebar.caption("Sayiyi yazip Enter'a bas (veya +/- oklarini kullan). "
                   "Adetler adrese islenir; sayfayi yer imine eklersen "
                   "sonraki aciliste otomatik gelir.")
