...
FONLAR = ["TLY", "THF", "DOH", "DFI", "BSM",          # serbest & hisse
          "PNU", "PRY", "TP2", "GAL", "GTL"]          # para piyasasi (sabit sira)
GUN_ADLARI = {0: "Pzt", 1: "Sali", 2: "Cars", 3: "Pers", 4: "Cuma"}

YEDEK_SEPETLER = {
    # DFI: Fintables/KAP Agustos 2026 raporu (03.09.2026 dogrulandi)
    "DFI": {"IEYHO": 50.25, "ISKPL": 2.99, "LIDER": 0.06, "BKRGY": 0.02},
...
MEVDUAT_BENZERI = ["PNU", "PRY", "TP2", "GAL", "GTL"]
RISK_YEDEK = {"TLY": 7, "THF": 6, "DOH": 6, "DFI": 7, "BSM": 7,
              "GAL": 1, "GTL": 1, "TP2": 1, "PRY": 1, "PNU": 1}
...
# Her fonun sabit mesaj rengi (daire = serbest/hisse, kare = para piyasasi)
FON_RENK = {"TLY": "🔵", "THF": "🟣", "DOH": "🔴", "DFI": "🟠", "BSM": "🟡",
            "PNU": "🟩", "PRY": "🟦", "TP2": "🟨", "GAL": "🟪", "GTL": "🟧"}

# --- SPK 28.08.2026 serbest fon duzenlemesi icin sabitler ---
SERBEST_FONLAR = ["TLY", "DOH", "DFI", "BSM"]              # yeni kurallara dogrudan tabi
...
# Hisse -> sektor (FVT tarzi sektorel dagilim icin; bilinmeyenler "Diger")
SEKTOR_YEDEK = {
    "DSTKF": "Faktoring/Finans", "LIDER": "Faktoring/Finans",
    "OZATD": "Denizcilik/Tasima", "THYAO": "Havacilik", "PGSUS": "Havacilik",
    "TEHOL": "Holding", "TRHOL": "Holding", "TERA": "Araci Kurum",
    "RALYH": "Holding", "HEDEF": "Holding", "GLRMK": "Holding",
    "PEKGY": "GYO", "SVGYO": "GYO", "EKGYO": "GYO",
    "BIGEN": "Enerji", "GESAN": "Enerji", "EUPWR": "Enerji",
    "KONTR": "Enerji", "MANAS": "Enerji Teknoloji", "ANELE": "Elektrik/Taahhut",
    "SELEC": "Saglik/Ecza", "ALKLC": "Gida", "ATATR": "Turizm",
    "ASELS": "Savunma", "SISE": "Cam/Sanayi", "KARCL": "Demir-Celik",
    "EREGL": "Demir-Celik", "KRDMD": "Demir-Celik", "BARMA": "Ambalaj",
    "NETCD": "Teknoloji", "TMPOL": "Kimya", "TUPRS": "Enerji/Rafinaj",
    "AKBNK": "Banka", "ISCTR": "Banka", "YKBNK": "Banka", "HALKB": "Banka",
    "GARAN": "Banka", "VAKBN": "Banka", "TCELL": "Iletisim",
    "TTKOM": "Iletisim", "BRSAN": "Boru/Sanayi",
}

_DONUT_RENKLER = ["#1f4e79", "#2e86ab", "#d9a441", "#12897b", "#c0504d",
                  "#7b5ea7", "#e8743b", "#5b9bd5", "#70ad47", "#8c6d31",
                  "#6b7b8c", "#b55f8f", "#4b8bbe", "#a0522d"]

def donut_ciz(veri, baslik, yukseklik=205):
    """{ad: pay} icin daire grafik + altinda yuzdeli renkli lejant."""
    pozitif = {a: round(v, 2) for a, v in (veri or {}).items() if v > 0.01}
    negatif = {a: v for a, v in (veri or {}).items() if v < -0.01}
    if not pozitif:
        return False
    sirali = sorted(pozitif.items(), key=lambda x: -x[1])
    adlar = [a for a, _ in sirali]
    renkler = [_DONUT_RENKLER[i % len(_DONUT_RENKLER)]
               for i in range(len(adlar))]
    st.markdown(f"**{baslik}**")
    d_df = pd.DataFrame({"Kalem": adlar, "Pay": [v for _, v in sirali]})
    grafik = (alt.Chart(d_df).mark_arc(innerRadius=46, cornerRadius=3,
                                       padAngle=0.012).encode(
        theta=alt.Theta("Pay:Q", sort=None),
        color=alt.Color("Kalem:N", sort=None, legend=None,
                        scale=alt.Scale(domain=adlar, range=renkler)),
        tooltip=[alt.Tooltip("Kalem:N"),
                 alt.Tooltip("Pay:Q", format=".2f", title="%")])
        .properties(height=yukseklik))
    st.altair_chart(grafik, use_container_width=True)
    parcalar = []
    for (a, v), r in zip(sirali, renkler):
        deger = f"{v:.2f}".replace(".", ",")
        parcalar.append(
            "<span style='display:inline-flex;align-items:center;"
            "margin:0 14px 6px 0;font-size:.88rem'>"
            f"<span style='width:10px;height:10px;border-radius:3px;"
            f"background:{r};display:inline-block;margin-right:6px'></span>"
            f"{a}: <b>&nbsp;%{deger}</b></span>")
    st.markdown("<div style='line-height:1.7'>" + "".join(parcalar) + "</div>",
                unsafe_allow_html=True)
    for a, v in sorted(negatif.items(), key=lambda x: x[1]):
        st.markdown(f"<span style='color:#c62828;font-size:.88rem'>▼ {a}: "
                    f"<b>%{f'{v:.2f}'.replace('.', ',')}</b></span>",
                    unsafe_allow_html=True)
    return True

def sektor_dagilimi(kod):
    """Fon sepetini sektorlere toplar."""
    sepet = SEPETLER.get(kod, {})
    dagilim = {}
    for h, w in sepet.items():
        s = SEKTOR_YEDEK.get(h, "Diger")
        dagilim[s] = dagilim.get(s, 0) + w
    return dict(sorted(dagilim.items(), key=lambda x: -x[1]))
...
/* --- Kenar cubugu ac/kapa dugmeleri: TUM surum kimlikleri kapsanir --- */
[data-testid="stExpandSidebarButton"],
[data-testid="stSidebarCollapsedControl"],
[data-testid="collapsedControl"] {
  background: linear-gradient(120deg, #1f4e79, #2e86ab) !important;
...
[data-testid="stSidebarCollapsedControl"]:hover {transform: scale(1.06);}
...
[data-testid="stExpandSidebarButton"]::after {
  ...
  font-size: .92rem; margin-left: 6px; white-space: nowrap;
}
/* --- GENIS RENK TURU --- */
h2 {
  background: linear-gradient(120deg, #1f4e79 20%, #2e86ab 60%, #b8860b);
  -webkit-background-clip: text; background-clip: text;
  color: transparent !important;
}
div[data-baseweb="tab-list"] {
  background: linear-gradient(90deg, #eef4fb, #fdf6e8);
...
[data-testid="stDataFrame"] {
  ...
  background: linear-gradient(#fff, #fff) padding-box,
              linear-gradient(120deg, #2e86ab, #7bc4a4, #d9a441) border-box !important;
}
[data-testid="stMetric"] {
  background: linear-gradient(180deg, #ffffff, #f7fafd);
}
.stButton button, .stDownloadButton button, .stFormSubmitButton button {
  border: 1.5px solid #2e86ab !important; color: #1f4e79;
  transition: all .15s ease;
}
...
section[data-testid="stSidebar"] {
  background: linear-gradient(180deg, #e8f1fb 0%, #f2f7fd 40%, #fdf8ee 100%);
}
section[data-testid="stSidebar"] div[data-testid="stExpander"] {
  background: #ffffff; border-radius: 12px;
  box-shadow: 0 1px 4px rgba(16,24,40,.06);
}
hr {border-color: #2e86ab33;}
/* eski kural blogu: */
/* --- Kenar cubugu (eski kimlikler) --- */
section[data-testid="stSidebar"] button[kind="headerNoPadding"],
[data-testid="stSidebarCollapseButton"] button {
  background: linear-gradient(120deg, #1f4e79, #2e86ab) !important;
  border-radius: 12px !important;
  box-shadow: 0 3px 10px rgba(31,78,121,.35) !important;
  transition: transform .15s ease;
}
...
div[data-testid="stExpander"] summary:hover {color: #1f4e79;}
button[data-baseweb="tab"] {
  background: #eef4fb;
}
section[data-testid="stSidebar"] h1 {
  background: linear-gradient(120deg, #1f4e79, #2e86ab, #d9a441);
  -webkit-background-clip: text; background-clip: text;
  color: transparent !important;
}
[data-testid="stMetricValue"] {color: #16283f;}
.stApp {background: linear-gradient(180deg, #edf4fc 0%, #fdfefe 320px);}
/* Grafik ve tablolarin sag ust arac cubugu (tablo gorunumu/buyutme) kapali:
   yanlislikla acilip kapatilamayan veri tablosu sorunumu bitirir */
...
with st.sidebar.expander("⚙️ Genel ayarlar", expanded=False):
    duraklat = st.toggle("⏸ Yenilemeyi durdur (adet girerken ac)",
                                 value=False)
    oto_sepet = st.toggle("🤖 Sepetleri KAP'tan otomatik al", value=True,
                                  help="Kapatirsan guvenilir yedek listeler "
                                       "kullanilir. Sorun aninda kapat.")
    aralik = st.slider("Yenileme araligi (dakika)", 1, 30, 5)
    GUN_MOD = st.radio("Gun sutunlari icerigi",
                               ["% Degisim", "Kapanis TL"],
...
with st.sidebar.expander("➕ Fon ekle / cikar"):
    with st.form("ekfon_formu"):
        ...
        fon_temizle = False   # tek tek cikarma asagida
    if fon_ekle and yeni_fon.strip():
        ...
        if 2 <= len(kod_yeni) <= 6 and kod_yeni not in FONLAR:
            ...
            st.rerun()
    if EK_FONLAR:
        st.caption("Eklediklerin — cikarmak icin ❌:")
        for _ef in EK_FONLAR:
            c_ef1, c_ef2 = st.columns([3, 1])
            c_ef1.markdown(f"**{_ef}**")
            if c_ef2.button("❌", key=f"cikar_{_ef}",
                            use_container_width=True):
                KISISEL["ekfon"] = [f for f in EK_FONLAR if f != _ef]
                kisisel_kaydet(KISISEL)
                st.rerun()
...
with st.sidebar.form("portfoy_formu"):
    st.caption("Adet · alis fiyati · alis tutari — ucunden IKISINI doldur, "
               "kalanini Kaydet'te ben hesaplarim.")
    _pf_satirlar = []
    for _kod in FONLAR:
        try:
        ...
        try:
            ...
            _mvars = 0.0
        _pf_satirlar.append({"Fon": _kod, "Adet": _vars,
                             "Alis Fiyati": _mvars,
                             "Alis Tutari": round(_vars * _mvars, 2)})
    _pf_duzen = st.data_editor(
        pd.DataFrame(_pf_satirlar), hide_index=True,
        use_container_width=True, disabled=["Fon"],
        key=f"pf_ed_{st.session_state.get('pf_sifir', 0)}",
        column_config={
            "Fon": st.column_config.TextColumn("Fon", width="small"),
            "Adet": st.column_config.NumberColumn("Adet", min_value=0.0,
                                                  format="%.2f"),
            "Alis Fiyati": st.column_config.NumberColumn("Alis Fiyati",
                                                         min_value=0.0,
                                                         format="%.4f"),
            "Alis Tutari": st.column_config.NumberColumn("Alis Tutari",
                                                         min_value=0.0,
                                                         format="%.2f")})
    _girisler = {r["Fon"]: float(r["Adet"] or 0)
                 for _, r in _pf_duzen.iterrows()}
    _maliyet_girisler = {r["Fon"]: float(r["Alis Fiyati"] or 0)
                         for _, r in _pf_duzen.iterrows()}
    _tutar_girisler = {r["Fon"]: float(r["Alis Tutari"] or 0)
                       for _, r in _pf_duzen.iterrows()}
...
if _sifirla:
    ...
    st.session_state["pf_sifir"] = st.session_state.get("pf_sifir", 0) + 1
    kisisel_kaydet(KISISEL)
    ...
    _hesaplananlar = []
    for _kod in FONLAR:
        _a, _f, _t = (_girisler[_kod], _maliyet_girisler[_kod],
                      _tutar_girisler[_kod])
        if _a > 0 and _f > 0:
            pass                                   # tutar bilgi amacli
        elif _a > 0 and _t > 0:                    # fiyati hesapla
            _maliyet_girisler[_kod] = round(_t / _a, 6)
            _hesaplananlar.append(f"{_kod} alis fiyati "
                                  f"{tr_sayi(_t / _a, 4)} TL")
        elif _f > 0 and _t > 0:                    # adedi hesapla
            _girisler[_kod] = round(_t / _f, 4)
            _hesaplananlar.append(f"{_kod} adet {tr_sayi(_t / _f, 2)}")
    KISISEL["adet"] = {k: v for k, v in _girisler.items() if v > 0}
    KISISEL["maliyet"] = {k: v for k, v in _maliyet_girisler.items() if v > 0}
    if _hesaplananlar:
        st.sidebar.info("Otomatik hesaplandi: " + " · ".join(_hesaplananlar))
    kisisel_kaydet(KISISEL)
...
with st.sidebar.expander("❓ Portfoyum nasil kullanilir?"):
    st.markdown(
        "**1.** ⚙️ Genel ayarlar icindeki *Yenilemeyi durdur* anahtarini ac.\n\n"
...
with st.sidebar.expander("🔔 Alarm esiklerim (%)"):
    st.caption("Her fon icin iki esik: **dusus** (eksi) ve **yukselis** (arti). "
               "Istedigin sayiyi elle yaz; yukselis esigi 0 = kapali.")
    with st.form("esik_formu"):
        ...
        for _kod in FONLAR:
            ...
            try:
                _yv = float((KISISEL.get("esik_yuk") or {}).get(_kod, 0.0))
            except (TypeError, ValueError):
                _yv = 0.0
            _mad = st.session_state.get("karne_mad", {}).get(_kod)
            _oneri = ("—" if _mad is None
                      else f"{-math.ceil((1.0 + 2 * _mad) * 2) / 2:g}")
            _es_satirlar.append({"Fon": _kod, "Dusus": _ev,
                                 "Yukselis": _yv, "Oneri": _oneri})
        _es_duzen = st.data_editor(
            pd.DataFrame(_es_satirlar), hide_index=True,
            use_container_width=True, disabled=["Fon", "Oneri"],
            key="esik_ed",
            column_config={
                "Fon": st.column_config.TextColumn("Fon", width="small"),
                "Dusus": st.column_config.NumberColumn("Dusus",
                                                       min_value=-99.0,
                                                       max_value=0.0,
                                                       format="%.1f"),
                "Yukselis": st.column_config.NumberColumn("Yukselis",
                                                          min_value=0.0,
                                                          max_value=99.0,
                                                          format="%.1f"),
                "Oneri": st.column_config.TextColumn("Oneri", width="small",
                                                     help="Tahmin Karnesi "
                                                          "sapmasina gore "
                                                          "onerilen dusus "
                                                          "esigi")})
        _esikler = {r["Fon"]: float(r["Dusus"] or 0)
                    for _, r in _es_duzen.iterrows()}
        _esikler_yuk = {r["Fon"]: float(r["Yukselis"] or 0)
                        for _, r in _es_duzen.iterrows()}
        if st.form_submit_button("💾 Esikleri kaydet",
                                 use_container_width=True):
            KISISEL["esik"] = {k: v for k, v in _esikler.items()
                               if abs(v - VARSAYILAN_ESIK[k]) > 1e-9}
            KISISEL["esik_yuk"] = {k: v for k, v in _esikler_yuk.items()
                                   if v > 0}
            kisisel_kaydet(KISISEL)
            st.success("Esikler bu cihaza kaydedildi ✅")
    st.caption("Dusus ornegi: -3,5 → gunluk %-3,5 ve altinda alarm. "
               "Yukselis: 2 → %+2 ve ustunde alarm (0 = kapali). Oneri "
               "kolonu Tahmin Karnesi'nden gelir; esigin oneriden buyuk "
               "(sifira yakin) olmasi gurultuye takilabilir.")
KULLANICI_ESIK = _esikler
KULLANICI_ESIK_YUK = _esikler_yuk
...
_VARLIK_ADLARI = {"HISSE SENETLER": "Hisse Senedi",
                  ...
                  "T.REPO": "Ters Repo", "TERS REPO": "Ters Repo",
                  "VADELI MEVDUAT": "Mevduat", "MEVDUAT": "Mevduat",
                  "REPO": "Repo",
...
def _java_pdf_coz(ham):
    """KAP ekleri Java serilestirilmis byte[] icinde gelir; icinden PDF'i cikarir."""
    if ham[:2] == b"\xac\xed":
        try:
            i = ham.index(b"\x78\x70", 10)
            n = int.from_bytes(ham[i + 2:i + 6], "big")
            return ham[i + 6:i + 6 + n]
        except Exception:
            pass
    return ham
...
@st.cache_data(ttl=1800, show_spinner=False)
def _kap_isit():
    """WAF icin oturum isitma (dokumante edilmis oneri)."""
    try:
        requests.get("https://www.kap.org.tr/tr/bildirim-sorgu",
                     headers=_KAP_BASLIK, timeout=8)
    except Exception:
        pass
    return True
...
def kap_fon_oid(kod):
    """TEFAS fon kodundan KAP uye kimligini (mkkMemberOid) bulur."""
    if kod in FON_KAP_ID:
        return FON_KAP_ID[kod]
    try:
        b = dict(_KAP_BASLIK)
        b["Referer"] = "https://www.kap.org.tr/tr/bildirim-sorgu"
        b["Accept"] = "application/json"
        r = requests.get(f"https://www.kap.org.tr/tr/api/member/filter/{kod}",
                         headers=b, timeout=10)
        if r.status_code != 200:
            return None
        v = r.json()
        if isinstance(v, list):
            v = v[0] if v else {}
        return (v or {}).get("mkkMemberOid")
    except Exception:
        return None

def _kap_norm(s):
    return (str(s or "").upper().replace("İ", "I").replace("Ö", "O")
            .replace("Ü", "U").replace("Ğ", "G"))

def _kap_liste_sorgu(govde, tani):
    b = dict(_KAP_BASLIK)
    b["Accept"] = "application/json"
    b["Referer"] = "https://www.kap.org.tr/tr/bildirim-sorgu"
    r = requests.post(
        "https://www.kap.org.tr/tr/api/disclosure/members/byCriteria",
        json=govde, headers=b, timeout=20)
    if r.status_code != 200:
...
def _kap_genel_pdr(_gun_damgasi):
    """Son 8 gunun TUM bildirimlerini tek sorguda ceker; fon kodu ->
    en yeni Portfoy Dagilim Raporu kaydi sozlugu dondurur. Kimlik gerekmez."""
    _kap_isit()
    tani_g = []
    kayitlar = _kap_liste_sorgu(
        {"fromDate": (simdi_tr() - timedelta(days=8)).strftime("%Y-%m-%d"),
         "toDate": simdi_tr().strftime("%Y-%m-%d"),
         "mkkMemberOidList": [], "subjectList": []}, tani_g)
    havuz = {}
    for k in (kayitlar or []):
        fk = (k.get("fundCode") or "").strip().upper()
        if not fk:
            continue
        if "PORTFOY DAGILIM" not in _kap_norm(k.get("subject")) and \
                "PORTFOY DAGILIM" not in _kap_norm(k.get("summary")):
            continue
        try:
            t = datetime.strptime(k.get("publishDate", "01.01.2000 00:00:00"),
                                  "%d.%m.%Y %H:%M:%S")
        except ValueError:
            t = datetime(2000, 1, 1)
        if fk not in havuz or t > havuz[fk][0]:
            havuz[fk] = (t, k)
    return {fk: k for fk, (t, k) in havuz.items()}, \
           (tani_g[0] if tani_g else f"{len(havuz)} fonun PDR'i bulundu")

def _kap_haftalik(kod, tani):
    """KAP JSON API: once kimlikli dar sorgu, olmazsa genel PDR havuzu
    (fundCode ile). Tum fonlar icin calisir."""
    _kap_isit()
    secim = None
    oid = kap_fon_oid(kod)
    if oid:
        try:
            kayitlar = _kap_liste_sorgu(
                {"fromDate": (simdi_tr() - timedelta(days=45)).strftime("%Y-%m-%d"),
                 "toDate": simdi_tr().strftime("%Y-%m-%d"),
                 "mkkMemberOidList": [oid], "subjectList": []}, tani) or []
            pdr = [k for k in kayitlar
                   if "PORTFOY DAGILIM" in _kap_norm(k.get("subject"))
                   or "PORTFOY DAGILIM" in _kap_norm(k.get("summary"))]
            pdr.sort(key=lambda k: datetime.strptime(
                k.get("publishDate", "01.01.2000 00:00:00"),
                "%d.%m.%Y %H:%M:%S"), reverse=True)
            secim = pdr[0] if pdr else None
            if secim is None:
                tani.append(f"KAP kimlikli listede PDR yok ({len(kayitlar)})")
        except Exception as e:
            tani.append(f"KAP kimlikli sorgu: {type(e).__name__}")
    if secim is None:
        try:
            havuz, notu = _kap_genel_pdr(simdi_tr().strftime("%Y-%m-%d-%H")[:13])
            secim = havuz.get(kod)
            if secim is None:
                tani.append(f"KAP genel havuz: {kod} yok ({notu})")
        except Exception as e:
            tani.append(f"KAP genel havuz: {type(e).__name__}")
    if secim is None:
        # Sonda merdiveni: fon bildirimleri farkli uclarda olabilir;
        # her adayin cevabini teshise yaz ki dogru kapiyi kesin bulalim
        b_p = dict(_KAP_BASLIK)
        b_p["Accept"] = "application/json"
        b_p["Referer"] = "https://www.kap.org.tr/tr/bildirim-sorgu"
        govde_p = {"fromDate": (simdi_tr() - timedelta(days=10)).strftime("%Y-%m-%d"),
                   "toDate": simdi_tr().strftime("%Y-%m-%d"),
                   "mkkMemberOidList": [], "subjectList": []}
        # 1) fon kimligi: fund/filter/{kod}
        f_oid = None
        try:
            rp = requests.get(
                f"https://www.kap.org.tr/tr/api/fund/filter/{kod}",
                headers=b_p, timeout=10)
            if rp.status_code == 200:
                v = rp.json()
                if isinstance(v, list):
                    v = v[0] if v else {}
                f_oid = (v or {}).get("mkkMemberOid") or (v or {}).get("fundOid")
            tani.append(f"P1 fund/filter: {rp.status_code}"
                        + (f" oid✓" if f_oid else ""))
        except Exception as e:
            tani.append(f"P1 fund/filter: {type(e).__name__}")
        # 2) fon oid ile dar sorgu
        if f_oid:
            try:
                kay = _kap_liste_sorgu(dict(govde_p,
                                            mkkMemberOidList=[f_oid]), tani) or []
                pdr = [k for k in kay
                       if "PORTFOY DAGILIM" in _kap_norm(k.get("subject"))
                       or "PORTFOY DAGILIM" in _kap_norm(k.get("summary"))]
                tani.append(f"P2 oid sorgu: {len(kay)} kayit, {len(pdr)} PDR")
                if pdr:
                    pdr.sort(key=lambda k: k.get("publishDate", ""),
                             reverse=True)
                    secim = pdr[0]
            except Exception as e:
                tani.append(f"P2 oid sorgu: {type(e).__name__}")
        # 3) fonlara ozel liste ucu
        if secim is None:
            try:
                rp3 = requests.post(
                    "https://www.kap.org.tr/tr/api/disclosure/funds/byCriteria",
                    json=govde_p, headers=b_p, timeout=15)
                if rp3.status_code == 200:
                    kay3 = rp3.json() or []
                    pdr3 = [k for k in kay3 if isinstance(k, dict)
                            and ((k.get("fundCode") or "").strip().upper() == kod
                                 or kod in _kap_norm(k.get("kapTitle")))
                            and ("PORTFOY DAGILIM" in _kap_norm(k.get("subject"))
                                 or "PORTFOY DAGILIM" in _kap_norm(k.get("summary")))]
                    tani.append(f"P3 funds/byCriteria: 200, "
                                f"{len(kay3)} kayit, {len(pdr3)} PDR")
                    if pdr3:
                        pdr3.sort(key=lambda k: k.get("publishDate", ""),
                                  reverse=True)
                        secim = pdr3[0]
                else:
                    tani.append(f"P3 funds/byCriteria: {rp3.status_code}")
            except Exception as e:
                tani.append(f"P3 funds/byCriteria: {type(e).__name__}")
    if secim is None:
        return None, None
    b = dict(_KAP_BASLIK)
    b["Accept"] = "application/json"
    try:
        idx = secim.get("disclosureIndex")
        donem_m = str(secim.get("period") or "")
        yil = str(secim.get("year") or simdi_tr().year)
        m_h = re.search(r"(\d{1,2})", donem_m)
        if "Hafta" in donem_m and m_h:
            etiket = f"{yil}.H{int(m_h.group(1)):02d}"
        elif "Ayl" in donem_m and m_h:
            etiket = f"{yil}.{int(m_h.group(1)):02d}"
        else:
            etiket = f"KAP-{idx}"
        b2 = dict(b)
        b2["Referer"] = f"https://www.kap.org.tr/tr/Bildirim/{idx}"
        r2 = requests.get("https://www.kap.org.tr/tr/api/notification/"
                          f"attachment-detail/{idx}", headers=b2, timeout=12)
        if r2.status_code != 200:
            tani.append(f"KAP detay HTTP {r2.status_code}")
            return None, None
        det = r2.json()
        if isinstance(det, list):
            det = det[0] if det else {}
        ekler = (det or {}).get("attachments") or []
        if not ekler:
            tani.append(f"KAP bildirim {idx}: ek dosya yok")
            return None, None
        obj = ekler[0].get("objId")
        r3 = requests.get(f"https://www.kap.org.tr/tr/api/file/download/{obj}",
                          headers=b2, timeout=20)
        pdf = _java_pdf_coz(r3.content)
        if r3.status_code != 200 or b"%PDF" not in pdf[:2048]:
            tani.append(f"KAP ek HTTP {r3.status_code} / PDF degil")
            return None, None
        return pdf, etiket
    except Exception as e:
        tani.append(f"KAP: {type(e).__name__}: {str(e)[:60]}")
        return None, None

AY_ADLARI = {"OCAK": 1, "SUBAT": 2, "MART": 3, "NISAN": 4, "MAYIS": 5,
             "HAZIRAN": 6, "TEMMUZ": 7, "AGUSTOS": 8, "EYLUL": 9,
             "EKIM": 10, "KASIM": 11, "ARALIK": 12}
_VARLIK_SAYFA_ONEK = ("HISSE", "YATIRIM FON", "MEVDUAT", "FINANSMAN",
                      "TERS", "REPO", "BORCLANMA", "DOVIZ", "KIRA",
                      "VIOP", "VADELI", "DIGER", "KAMU", "OZEL", "KIYMETLI")

def _fintables_sayfasi(kod, tani):
    """Fintables fon sayfasindan pozisyonlar + varlik dagilimi + donem.
    Sayfa her fonda ayni sablonda oldugu icin genel calisir."""
    def _n(s):
        return (s.upper().replace("İ", "I").replace("Ş", "S")
                .replace("Ğ", "G").replace("Ü", "U").replace("Ö", "O")
                .replace("Ç", "C"))
    try:
        r = requests.get(f"https://fintables.com/fonlar/{kod}",
                         headers={"User-Agent": TARAYICI["User-Agent"],
                                  "Accept": "text/html,application/xhtml+xml",
                                  "Accept-Language": "tr-TR,tr;q=0.9",
                                  "Referer": "https://fintables.com/fonlar/getiri"},
                         timeout=12)
        if r.status_code != 200:
            tani.append(f"Fintables sayfa: HTTP {r.status_code} "
                        f"({len(r.content)} bayt)")
            return {}, {}, None
        metin = r.text
        # Pozisyonlar: /sirketler/KOD linkini izleyen ilk yuzde = agirlik
        ag = {}
        for m in re.finditer(r'/sirketler/([A-Z]{4,6})', metin):
            h = m.group(1)
            if h in ag or h in _SEPET_KARA_LISTE:
                continue
            parca = metin[m.end():m.end() + 450]
            y = re.findall(r'%\s*(-?\d{1,2},\d{1,2})', parca)
            if not y:
                continue
            try:
                w = float(y[0].replace(",", "."))
            ...
            if 0 < w <= 60:
                ag[h] = w
        # Varlik dagilimi: "Hisse Senedi (%56,16)" kaliplari
        dagilim = {}
        for ad, deger in re.findall(
                r'([A-Za-zÇĞİÖŞÜçğıöşü ./()-]{3,45}?)\s*\(%(-?\d{1,3},\d{1,2})\)',
                metin):
            ad_t = ad.strip().lstrip(">").strip()
            if not _n(ad_t).startswith(_VARLIK_SAYFA_ONEK):
                continue
            try:
                dagilim[ad_t] = float(deger.replace(",", "."))
            except ValueError:
                continue
        # Donem: "aciklanan Agustos 2026 portfoy dagilim raporu"
        donem = None
        m_d = re.search(r'a[cç][ıi]klanan[^<]{0,40}?([A-Za-zÇĞİÖŞÜçğıöşü]+)'
                        r'\s+(\d{4})\s+portf', metin, re.IGNORECASE)
        if m_d:
            ay_no = AY_ADLARI.get(_n(m_d.group(1)))
            if ay_no:
                donem = f"{m_d.group(2)}.{ay_no:02d}"
        toplam = sum(ag.values())
        if ag and 0 < toplam <= 110:
            return ag, dagilim, donem
        tani.append(f"Fintables sayfa: {len(ag)}h/%{toplam:.0f} ({len(metin)} bayt)")
        return {}, dagilim, donem
    except Exception as e:
        tani.append(f"Fintables sayfa: {type(e).__name__}")
        return {}, {}, None

AYIKLAYICI_SURUM = 18   # parser her degistiginde artir -> onbellek yenilenir

@st.cache_data(ttl=86400, show_spinner=False)
def sepet_otomatik(kod, _surum=AYIKLAYICI_SURUM, elle_url=""):
    """(agirliklar, donem, tani, dagilim). Sira: elle link -> KAP -> Fintables."""
    tani = []
    try:
    ...
    # 0) Kullanicinin yapistirdigi KAP rapor eki linki (en guvenilir yol)
    from urllib.parse import urlparse as _up
    _eh = _up(elle_url).netloc.lower() if elle_url else ""
    if elle_url and elle_url.startswith("https://") and (
            _eh.endswith("kap.org.tr") or _eh.endswith("fintables.com")):
        try:
            r0 = requests.get(elle_url, headers=_KAP_BASLIK, timeout=15)
            _p0 = _java_pdf_coz(r0.content)
            if r0.status_code == 200 and b"%PDF" in _p0[:2048]:
                ag, notu, dagilim = _pdf_sepet_coz(_p0)
                if ag:
                    return (ag, "elle-link",
                            f"elle link: OK — {notu}", dagilim)
                tani.append(f"elle link: PDF cozulemedi [{notu}]")
            else:
                tani.append(f"elle link: HTTP {r0.status_code} / PDF degil")
        except Exception as e:
            tani.append(f"elle link: {type(e).__name__}")
    ...
    if pdf_icerik:
        try:
            ...
            son_dagilim = dagilim or son_dagilim
            if ag:
    ...
    ag_s, dagilim_s, donem_s = _fintables_sayfasi(kod, tani)
    if ag_s:
        donem = donem_s or simdi.strftime("%Y.%m")
        return (ag_s, donem,
                f"{donem}: OK — Fintables sayfasi ({len(ag_s)} hisse, "
                f"%{sum(ag_s.values()):.0f}; raporun ilk pozisyonlari)",
                dagilim_s)
    return None, None, (" | ".join(tani) if tani else "denenmedi"), son_dagilim

SEPET_TANI = {}
VARLIK_DAGILIMI = {}
SEPET_KAYNAK = {}
for _kod in list(SEPETLER):
    _elle = (KISISEL.get("elle_rapor") or {}).get(_kod, "")
    _oto, _donem, _tani, _dagilim = (sepet_otomatik(_kod, AYIKLAYICI_SURUM, _elle)
...
# Cekirdek fonlardan adi bilinmeyenlerin adini/turunu cek (BSM/GAL/GTL)
for _kod in FONLAR:
    if _kod in FON_ACIKLAMA or _kod in EK_FONLAR:
        continue
    _ad, _tur = fon_bilgisi(_kod)
    if _ad:
        FON_ACIKLAMA[_kod] = _ad
    if ("para piyasas" in _tur.lower() or "kisa vadeli" in _tur.lower()) \
            and _kod not in MEVDUAT_BENZERI:
        MEVDUAT_BENZERI.append(_kod)
        if _kod in SERBEST_FONLAR:
            SERBEST_FONLAR.remove(_kod)
...
for kod in FONLAR:
    ...
    # Ek fonlar Actions arsivinde olmadigi icin son verilerini bu cihazda sakla
    if kod in EK_FONLAR:
        if t is not None and kaynak and not (kaynak.startswith("onbellek")
                                             or kaynak.startswith("arsiv")):
            try:
                _kayit = t.copy()
                _kayit["TARIH"] = _kayit["TARIH"].dt.strftime("%Y-%m-%d")
                _yeni_json = _kayit[[c for c in ["TARIH", "FIYAT", "KISISAYISI",
                                                 "PORTFOYBUYUKLUK"]
                                     if c in _kayit]].to_json(orient="records")
                _eski_json = (KISISEL.get("fon_arsiv") or {}).get(kod)
                if (_yeni_json != _eski_json
                        and "kisisel" in st.session_state):
                    KISISEL.setdefault("fon_arsiv", {})[kod] = _yeni_json
                    st.session_state["ekfon_arsiv_degisti"] = True
            except Exception:
                pass
        elif t is None:
            try:
                _ham = (KISISEL.get("fon_arsiv") or {}).get(kod)
                if _ham:
                    t = _kayitlardan_df(json.loads(_ham))
                    if t is not None:
                        kaynak = (f"cihaz arsivi "
                                  f"({t['TARIH'].iloc[-1].strftime('%d.%m')} verisi)")
...
if st.session_state.pop("ekfon_arsiv_degisti", False):
    kisisel_kaydet(KISISEL)
...
for _kod, _sepet in SEPETLER.items():
    ...
    if _donem and _donem not in SEPET_GECMISI.get(_kod, {}):
        ...
        _yeni_kayit = True
if _yeni_kayit and "kisisel" in st.session_state:
    # kisisel veriler tarayicidan okunmadan otomatik kayit yazma —
    # yoksa bos sozluk localStorage'a yazilip ekli fonlari silebilir
    kisisel_kaydet(KISISEL)
...
if KANAL_LISTESI:
    ...
    for kod in FONLAR:
        ...
        if gunluk is not None and gunluk <= esik \
            ...
            alarmlar.append(f"{FON_RENK.get(kod, '🔔')} *{kod}* gunluk "
                            f"%{gunluk:+.2f} — dusus esigin (%{esik:g}) "
                            "asildi.")
            gidenler[bugun_a].append(f"{kod}-esik")
        yuk = KULLANICI_ESIK_YUK.get(kod, 0)
        if gunluk is not None and yuk > 0 and gunluk >= yuk \
                and f"{kod}-esikyuk" not in gidenler[bugun_a]:
            alarmlar.append(f"{FON_RENK.get(kod, '🔔')} *{kod}* gunluk "
                            f"%{gunluk:+.2f} — yukselis esigin (%{yuk:g}) "
                            "asildi.")
            gidenler[bugun_a].append(f"{kod}-esikyuk")
        sepet = SEPETLER.get(kod, {})
        if sepet and not METRIK.empty:
            ...
            if tabanlar and f"{kod}-taban" not in gidenler[bugun_a]:
                alarmlar.append(f"{FON_RENK.get(kod, '🔔')} *{kod}* "
                                "sepetinde taban: " + ", ".join(tabanlar))
        ...
        if kod in SEPETLER and t_a is not None and len(t_a) >= 2:
            ...
            if pd.notna(ky) and pd.notna(kt) and ky < 0 and kt < 0 \
                ...
                alarmlar.append(
                    f"{FON_RENK.get(kod, '🔔')} *{kod}*: yatirimci ve tutar "
                    f"birlikte eksi — cikis sinyali (Yat {int(ky):+,} · "
                    f"Tutar {kt/1e9:+.2f} mlr TL)".replace(",", "."))
...
def fon_gun_analizi(kod):
    ...
    ayar = []
    for h in sepet:
        _sec = METRIK.loc[METRIK["Hisse"] == h, "Gun"]
        if _sec.empty or _sec.isna().all():
            continue          # yeni/verisiz hisse: analiz disi birak
        if hareket_turu(float(_sec.iloc[0])) == "ayarlama?":
            ayar.append(h)
    if ayar:
...
with sekmeler[0]:
    ...
    degisim_kolonlari = ["Yat. Δ (gun)", "Yat. Δ (hafta)", "Yat. Δ (ay)",
                         ...
                         "K/Z (TL)"]
    def _fon_tablo_ciz(df_k, baslik, r1="#1f4e79", r2="#2e86ab"):
        if df_k.empty:
            return
        st.markdown(
            f"<span style='display:inline-block;padding:7px 18px;"
            f"border-radius:999px;background:linear-gradient(120deg,{r1},{r2});"
            f"color:#fff;font-weight:700;font-size:1.02rem;margin:6px 0;"
            f"box-shadow:0 3px 10px rgba(18,35,61,.25)'>{baslik}</span>",
            unsafe_allow_html=True)
        try:                       # Fon kolonu solda sabit (telefonda kaydirirken)
            kolon_ayar = {"Fon": st.column_config.TextColumn("Fon",
                                                             pinned=True)}
        except TypeError:
            kolon_ayar = None
        st.dataframe(df_k.style
                       .map(renk, subset=yuzde_kolonlari)
                       .map(isaret_renk, subset=degisim_kolonlari)
                       .map(risk_renk, subset=["Risk"]),
                     use_container_width=True, hide_index=True,
                     column_config=kolon_ayar,
                     height=40 * len(df_k) + 60)

    _para_maske = fon_df["Fon"].isin(MEVDUAT_BENZERI)
    _fon_tablo_ciz(fon_df[~_para_maske], "🔓 Serbest & Hisse Fonlari",
                   "#1f4e79", "#2e86ab")
    _fon_tablo_ciz(fon_df[_para_maske], "💵 Para Piyasasi Fonlari",
                   "#0f7a5c", "#2ea88a")
    ...
    donem = dk1.radio("Donem", ["1 Hafta", "1 Ay", "Tumu (45 gun)"],
                      horizontal=True, key="grafik_donem", index=1)
    kiyas_gorunum = dk2.radio("Gorunum",
                              ["Birikimli getiri", "Gunluk degisim"],
                              horizontal=True, key="grafik_gorunum", index=1)
    ...
    if getiri_cizgi:
        ...
        with g1:
            if kiyas_gorunum == "Gunluk degisim":
                st.markdown(f"**Gunluk degisimler — {donem} (%)**")
                gunluk_cizgi = {}
                for _k in FONLAR:
                    _t = FON_VERI.get(_k)
                    if _t is not None and len(_t) > 2:
                        gunluk_cizgi[_k] = (_t.tail(donem_gun)
                                            .set_index("TARIH")["Degisim"])
                if gunluk_cizgi:
                    st.line_chart(pd.DataFrame(gunluk_cizgi))
                    st.caption("Her nokta o gunun resmi degisimi (%); sifir "
                               "ustu kazancli, alti kayipli gun.")
            else:
...
STOPAJ_VARSAYILAN = {"THF": 0.0}   # hisse senedi yogun fon: stopaj %0

def maliyet_hesaplayici(kod):
    with st.expander("💰 Alis-Satis Maliyet Hesaplayici (stopaj & komisyon)"):
        t_m = FON_VERI.get(kod)
        guncel = (float(t_m["FIYAT"].iloc[-1])
                  if t_m is not None and len(t_m) else 0.0)
        adet_v = float((KISISEL.get("adet") or {}).get(kod, 0) or 0) or 100.0
        alis_v = (float((KISISEL.get("maliyet") or {}).get(kod, 0) or 0)
                  or guncel or 1.0)
        c1, c2, c3 = st.columns(3)
        adet = c1.number_input("Adet", min_value=0.0, value=adet_v,
                               step=1.0, key=f"mh_a_{kod}")
        alis = c2.number_input("Alis fiyati (TL)", min_value=0.0,
                               value=float(alis_v), step=0.0001,
                               format="%.4f", key=f"mh_b_{kod}")
        satis = c3.number_input("Satis fiyati (TL)", min_value=0.0,
                                value=float(guncel or alis_v), step=0.0001,
                                format="%.4f", key=f"mh_c_{kod}",
                                help="Varsayilan: son resmi fiyat")
        c4, c5 = st.columns(2)
        stopaj = c4.number_input("Stopaj orani (%)", 0.0, 40.0,
                                 value=STOPAJ_VARSAYILAN.get(kod, 17.5),
                                 step=0.5, key=f"mh_s_{kod}")
        kom = c5.number_input("Kurum islem komisyonu (%, varsa)", 0.0, 5.0,
                              value=0.0, step=0.05, key=f"mh_k_{kod}",
                              help="Cogu kurum TEFAS fon islemlerinde ek "
                                   "komisyon almaz; aliyorsa buraya yaz.")
        brut_alis, brut_satis = adet * alis, adet * satis
        kar = brut_satis - brut_alis
        stopaj_tl = max(0.0, kar) * stopaj / 100
        kom_tl = brut_satis * kom / 100
        net = brut_satis - stopaj_tl - kom_tl
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Brut satis", tr_sayi(brut_satis, 2) + " TL")
        m2.metric("Kar / Zarar", tr_sayi(kar, 2) + " TL",
                  delta=f"%{(kar / brut_alis * 100):+.2f}" if brut_alis else None)
        m3.metric("Toplam kesinti", "-" + tr_sayi(stopaj_tl + kom_tl, 2) + " TL",
                  delta=f"stopaj {tr_sayi(stopaj_tl, 2)} + komisyon "
                        f"{tr_sayi(kom_tl, 2)}", delta_color="off")
        m4.metric("Net ele gecen", tr_sayi(net, 2) + " TL")
        st.markdown(
            "<div class='analiz-kutu'><b>Genel mantik:</b> "
            "<b>1)</b> Stopaj <u>anaparadan veya satis tutarindan degil, "
            "yalnizca KAR'dan</u> kesilir — zarardaysan stopaj cikmaz. "
            "<b>2)</b> Yonetim ucreti ayrica odenmez; her gun fon fiyatinin "
            "icinden kesilir, gordugun fiyat net fiyattir. "
            "<b>3)</b> TEFAS'ta fon alim-satiminda cogu bankada ek islem "
            "komisyonu yoktur; varsa satis tutari uzerinden yuzdedir. "
            "<b>4)</b> Hisse senedi yogun fonlarda (orn. THF) stopaj %0'dir; "
            "serbest ve para piyasasi fonlarinda gercek kisiler icin ~%17,5. "
            "Oranlar mevzuatla degisebilir — kurumundan teyit et. Hesap "
            "bilgilendirme amaclidir.</div>", unsafe_allow_html=True)
...
def hisse_fonu_sekmesi(kod):
    ...
    try:
        g1, g2 = st.columns([3, 2])
        with g1:
            if t_fon is not None and len(t_fon) > 2 and "Degisim" in t_fon:
                _res = (float(t_fon["Degisim"].iloc[-1])
                        if pd.notna(t_fon["Degisim"].iloc[-1]) else None)
                st.metric("Bugunku resmi degisim",
                          ("%" + yuzde_str(_res)) if _res is not None else "—",
                          delta=t_fon["TARIH"].iloc[-1].strftime("%d.%m verisi"),
                          delta_color="off")
                _don = st.radio("Donem", ["1 Hafta", "1 Ay", "Tumu (45 gun)"],
                                index=1, horizontal=True, key=f"fgd_{kod}")
                _gorunum = st.radio("Gorunum", ["Fiyat", "Gunluk degisim (%)"],
                                    index=1, horizontal=True, key=f"fgg_{kod}")
                _g = {"1 Hafta": 6, "1 Ay": 22, "Tumu (45 gun)": 9999}[_don]
                _kesit = t_fon.tail(_g)
                if len(_kesit) >= 2 and pd.notna(_kesit["FIYAT"].iloc[0]):
                    _ilk = float(_kesit["FIYAT"].iloc[0])
                    _son_f = float(_kesit["FIYAT"].iloc[-1])
                    _dg = (_son_f / _ilk - 1) * 100
                    st.markdown(
                        f"**{_don} getirisi:** "
                        f"<span style='color:{'#2e7d32' if _dg >= 0 else '#c62828'};"
                        f"font-weight:700'>%{_dg:+.2f}</span>",
                        unsafe_allow_html=True)
                    if _gorunum == "Fiyat":
                        st.line_chart(_kesit.set_index("TARIH")["FIYAT"],
                                      height=240)
                    else:
                        st.bar_chart(_kesit.set_index("TARIH")["Degisim"],
                                     height=240)
                        st.caption("Cubuklar gunluk resmi degisim (%); "
                                   "yesil/kirmizi ayrimi grafik uzerinde "
                                   "isaretle gorulur.")
            else:
                st.caption("Fiyat grafigi icin resmi veri bekleniyor.")
        with g2:
            _vd = donut_ciz(VARLIK_DAGILIMI.get(kod, {}),
                            "Varlik Dagilimi (KAP raporu, FTD %)")
            _sd = donut_ciz(sektor_dagilimi(kod),
                            "Sektorel Dagilim (hisse sepeti, %)")
            if _vd or _sd:
                st.caption("Kaynak: fonun son KAP portfoy raporu. Sektorler "
                           "yaklasik eslesmedir; toplamlar %100'den sapabilir.")
            else:
                st.caption("Dagilim bu fonun raporundan cikarilamadi; bir "
                           "sonraki rapor cekiminde tekrar denenir.")
    except Exception as _gh:
        st.caption(f"Grafik bolumu bu turda cizilemedi "
                   f"({type(_gh).__name__}); asagidaki tablolar etkilenmez.")
    ...
    try:
        _h_ayar = {"Hisse": st.column_config.TextColumn("Hisse", pinned=True)}
    except TypeError:
        _h_ayar = None
...
def genel_fon_sekmesi(kod):
    ...
    with a1:
        ...
        st.line_chart(t.set_index("TARIH")["FIYAT"], height=230)
        st.markdown("**Gunluk getiri (%)**")
        st.bar_chart(t.dropna(subset=["Degisim"]).set_index("TARIH")["Degisim"],
                     height=200)
    with a2:
        if donut_ciz(VARLIK_DAGILIMI.get(kod, {}),
                     "Varlik Dagilimi (KAP raporu, FTD %)"):
            st.caption("Hisse sepeti bulunmasa da fonun icindeki varlik "
                       "siniflari KAP raporundan okundu.")
        else:
            st.caption("Varlik dagilimi raporu henuz cekilemedi; asagidaki "
                       "teshis satirina bak.")
    st.info("Bu fon kullanici tarafindan eklendi. Hisse sepeti su an "
            "cikarilamadi — sepet bulununca bu sekme TLY gibi tam analize "
            "(hisse tablosu, tahmin, karne, alarm) otomatik donusur.")
    _tani = SEPET_TANI.get(kod)
    if _tani:
        st.caption(f"🔎 Rapor cekim teshisi: {_tani}")
    if st.button(f"🔄 {kod} sepetini simdi tekrar dene",
                 key=f"tekrar_{kod}"):
        sepet_otomatik.clear()
        st.rerun()
    maliyet_hesaplayici(kod)
