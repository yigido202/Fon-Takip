...
def _fintables_sayfasi(kod, tani):
    """Son care: Fintables fon sayfasindaki pozisyon listesini kazir."""
    try:
        ...
        metin = r.text
        ag, gorulen = {}, set()
        # 1) Gomulu JSON kaliplari  2) HTML metin kaliplari
        kaliplar = [r'"(?:code|symbol)"\s*:\s*"([A-Z]{4,6})"[^}]{0,200}?'
                    r'"(?:weight|ratio|percentage)"\s*:\s*([0-9]{1,2}(?:\.[0-9]{1,2})?)',
                    r'\b([A-Z]{4,6})\b[^<>%]{0,60}?%\s*([0-9]{1,2}[.,][0-9]{1,2})']
        for kalip in kaliplar:
            for m in re.finditer(kalip, metin):
                h = m.group(1)
                if h in _SEPET_KARA_LISTE:
                    continue
                try:
                    w = round(float(m.group(2).replace(",", ".")), 2)
                except ValueError:
                    continue
                if 0 < w <= 60 and (h, w) not in gorulen:
                    gorulen.add((h, w))
                    ag[h] = ag.get(h, 0) + w
            if len(ag) >= 3 and 25 <= sum(ag.values()) <= 110:
                return ag
            ag, gorulen = {}, set()
        tani.append(f"Fintables sayfa: {len(ag)}h/%{sum(ag.values()):.0f}")
        return {}
    except Exception as e:
        tani.append(f"Fintables sayfa: {type(e).__name__}")
        return {}

AYIKLAYICI_SURUM = 16   # parser her degistiginde artir -> onbellek yenilenir
...
def sepet_otomatik(kod, _surum=AYIKLAYICI_SURUM, elle_url=""):
    ...
    ag = _fintables_sayfasi(kod, tani)
    if ag:
        donem = simdi.strftime("%Y.%m")
        return (ag, donem,
                f"{donem}: OK — Fintables sayfasi ({len(ag)} hisse, "
                f"%{sum(ag.values()):.0f}; yaklasik/ilk pozisyonlar)", {})
