...
with sekmeler[0]:
    ...
    with st.expander("📲 WhatsApp Bildirimleri Nasil Kurulur?"):
        st.markdown("""
Bu site aciksa uyarilari zaten goruyorsun; WhatsApp bildirimi ise **site
kapaliyken** telefonuna mesaj gelmesini saglar. Kurulum ~10 dakika, tamamen
ucretsiz ve tek seferlik:

**Adim 1 — CallMeBot'u aktive et (2 dk):**
1. callmebot.com adresindeki *"Free API for WhatsApp Messages"* sayfasini ac
   ve orada yazan guncel bot numarasini telefon rehberine kaydet.
2. WhatsApp'tan bu numaraya su mesaji gonder:
   `I allow callmebot to send me messages`
3. Bot sana *"Your APIKEY is 123456"* gibi bir anahtar dondurecek — bunu not al.

**Adim 2 — Anahtarlari GitHub kasasina koy (3 dk):**
1. GitHub'da repona gir → **Settings → Secrets and variables → Actions →
   New repository secret**
2. Iki secret olustur:
   - Ad: `WHATSAPP_PHONE` → Deger: `+905xxxxxxxxx` (kendi numaran, ulke koduyla)
   - Ad: `CALLMEBOT_APIKEY` → Deger: bottan gelen anahtar
3. Bu bilgiler sifreli kasada durur, kodda/sitede asla gorunmez.

**Adim 3 — Alarm dosyalarini yukle (3 dk):**
1. `alarm_kontrol.py` dosyasini reponun ana dizinine yukle.
2. `alarm.yml` dosyasini repoda `.github/workflows/` klasorune yukle
   (klasor yoksa: Add file → Create new file → isim kutusuna
   `.github/workflows/alarm.yml` yaz, icerigi yapistir).

**Adim 4 — Test et (1 dk):**
Repoda **Actions** sekmesi → *Fon Alarmlari* → **Run workflow** → mod olarak
`ozet` yaz ve calistir. 1-2 dakika icinde WhatsApp'ina gun sonu ozeti gelmeli.

**Neler gelir?** Hafta ici 10:00-17:30 arasi her 30 dakikada bir kontrol:
🔴 risk seviyesine gore dusus esigi asilirsa (7/7 fonlarda %-3,5 | 5-6/7'de
%-2,5 | 3-4/7'de %-1,5 | para fonlarinda eksi gun aninda), 🟠 sepette taban
hissesi olusursa, 🟡 fondan ayni gun hem yatirimci hem para cikisi olursa.
Ayni alarm gunde 1 kez gelir, spam olmaz. Her aksam 18:45'te de 📊 gun sonu
ozeti gelir.
