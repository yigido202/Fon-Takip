...
def tl_kisa(deger):
    """Buyuk TL tutarini isaretli kisa bicimde yazar: +2,1 mlr / -350 mn"""
    if deger is None or pd.isna(deger):
        return "—"
    mutlak = abs(deger)
    if mutlak >= 1e9:
...
BOS_EK = {"Son Veri": "—", "Yatirimci": "—", "Yatirimci Δ (gunluk)": "—",
          "Toplam Tutar (TL)": "—", "Tutar Δ (gunluk)": "—"}
...
for kod in FONLAR:
    ...
    son = t.iloc[-1]
    onceki = t.iloc[-2] if len(t) >= 2 else None
    ...
    # Yeni sutunlar: resmi verinin tarihi + gunluk degisimler
    satir["Son Veri"] = son["TARIH"].strftime("%d.%m")
    satir["Yatirimci"] = tr_sayi(son["KISISAYISI"])
    if onceki is not None and pd.notna(son["KISISAYISI"]) and pd.notna(onceki["KISISAYISI"]):
        fark = int(son["KISISAYISI"] - onceki["KISISAYISI"])
        satir["Yatirimci Δ (gunluk)"] = f"{fark:+,}".replace(",", ".")
    else:
        satir["Yatirimci Δ (gunluk)"] = "—"
    satir["Toplam Tutar (TL)"] = tr_sayi(son["PORTFOYBUYUKLUK"], 2)
    if onceki is not None and pd.notna(son["PORTFOYBUYUKLUK"]) and pd.notna(onceki["PORTFOYBUYUKLUK"]):
        satir["Tutar Δ (gunluk)"] = tl_kisa(son["PORTFOYBUYUKLUK"] - onceki["PORTFOYBUYUKLUK"])
    else:
        satir["Tutar Δ (gunluk)"] = "—"
    satirlar.append(satir)
...
def isaret_renk(v):
    """'+1.095' veya '-2,1 mlr' gibi isaretli metinleri renklendirir."""
    if isinstance(v, str) and v not in ("—", ""):
        if v.startswith("+"):
...
degisim_kolonlari = ["Yatirimci Δ (gunluk)", "Tutar Δ (gunluk)"]
st.dataframe(df.style
               .map(renk, subset=yuzde_kolonlari)
               .map(isaret_renk, subset=degisim_kolonlari),
             use_container_width=True, hide_index=True,
             height=40 * len(FONLAR) + 60)
st.caption("Son Veri: resmi TEFAS verisinin ait oldugu tarih (fon fiyatlari "
           "gun sonunda aciklanir, gun ici saat bilgisi yayinlanmaz — panelin "
           "kendi kontrol saati basliktadir). Δ sutunlari onceki resmi gune "
           "gore degisimdir.")
