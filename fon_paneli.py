    ...
    for donem in donemler:
        try:
            r = requests.get(
                ...
                headers={"User-Agent": TARAYICI["User-Agent"]}, timeout=15)
...
for _kod in list(SEPETLER):
    _oto, _donem = sepet_otomatik(_kod)
    if _oto:
        SEPETLER[_kod] = _oto
...
def hisse_fonu_sekmesi(kod):
    ...
    if t_fon is not None and len(t_fon) > 5 and not DEG.empty:
        ciftler = []
        for _, r in t_fon.iterrows():
            if pd.isna(r["Degisim"]):
                continue
            tarih = r["TARIH"].date()
            th = sepet_tahmini_tarih(kod, tarih)
            if th is not None:
                ciftler.append(dict(Tarih=r["TARIH"], Resmi=float(r["Degisim"]),
                                    Tahmin=th, Sapma=th - float(r["Degisim"])))
        if len(ciftler) >= 5:
            karne = pd.DataFrame(ciftler)
