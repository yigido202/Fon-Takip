...
GUN_MOD = st.sidebar.radio("Gun sutunlari icerigi",
                           ["% Degisim", "% · Kapanis TL"],
                           help="Kapanis modunda her gunun yaninda o "
                                "gunku fon fiyati da gorunur; bugunun "
                                "hucresi ~tahmini kapanisi gosterir.")
if not duraklat:
...
def whatsapp_gonder(tel, anahtar, metin):
    try:
        r = requests.get(
            ...
            params={"phone": tel, "apikey": anahtar, "text": metin},
            timeout=20)
...
def renk(v):
    if isinstance(v, str) and v not in ("—", ""):
        try:
            parca = v.split("·")[0].strip()
            s = float(parca.replace("~", "").replace(",", "."))
...
for kod in FONLAR:
    ...
    for gun_no, ad in GUN_ADLARI.items():
        ...
        if hedef in resmi_gunler:
            hucre = yuzde_str(resmi_gunler[hedef])
            if GUN_MOD == "% · Kapanis TL" and hedef in kapanislar:
                hucre += " · " + kap_str(kapanislar[hedef])
            satir[ad] = hucre
        else:
            th = sepet_tahmini_tarih(kod, hedef)
            ...
            if th is not None:
                hucre = yuzde_str(th, True)
                if GUN_MOD == "% · Kapanis TL" and hedef == bugun \
                        and pd.notna(son["FIYAT"]):
                    hucre += " · " + kap_str(float(son["FIYAT"]) * (1 + th / 100),
                                             tahmin=True)
                satir[ad] = hucre
            else:
                satir[ad] = "—"
...
if WA_TEL and WA_KEY:
    ...
    if st.session_state.pop("wa_ozet_iste", False):
        if whatsapp_gonder(WA_TEL, WA_KEY, ozet_metni()):
            st.sidebar.success("Ozet WhatsApp'ina gonderildi ✅")
        else:
            st.sidebar.error("Gonderilemedi — numara/anahtari kontrol et.")
    ...
    if alarmlar:
        whatsapp_gonder(WA_TEL, WA_KEY,
                        "⚠ *Fon Alarm* (" + simdi_tr().strftime("%d.%m %H:%M")
                        + " TR)\n" + "\n".join(alarmlar)
                        + "\n_Yatirim tavsiyesi degildir._")
        st.toast("WhatsApp alarmi gonderildi 📲")
