...
with st.sidebar.expander("❓ Portfoyum nasil kullanilir?"):
    st.markdown(
        ...
        "**Bu linki kimseyle paylasma** (portfoyun icinde).\n\n"
...
with st.sidebar.form("tg_formu"):
    tg_token_giris = st.text_input("Bot token", type="password",
                                   value=st.query_params.get("tgt", ""),
                                   placeholder="123456:ABC-DEF...")
    tg_chat_giris = st.text_input("Chat ID",
                                  value=st.query_params.get("tgc", ""),
...
if tg_bul and tg_token_giris.strip():
    try:
        ...
        for guncelleme in reversed(sonuclar):
            ...
            if mesaj and mesaj.get("chat", {}).get("id"):
                ...
                break
        if bulunan:
            st.query_params["tgt"] = tg_token_giris.strip()
            st.query_params["tgc"] = bulunan
    ...
    if tg_token_giris.strip() and tg_chat_giris.strip():
        st.query_params["tgt"] = tg_token_giris.strip()
        st.query_params["tgc"] = tg_chat_giris.strip()
    else:
        for _a in ("tgt", "tgc"):
            if _a in st.query_params:
                del st.query_params[_a]
TG_TOKEN = st.query_params.get("tgt", "")
TG_CHAT = st.query_params.get("tgc", "")
if tg_test:
...
KISISEL_PARAMLAR = ([k for k in FONLAR] + [f"{k}m" for k in FONLAR]
                    + [f"{k}e" for k in FONLAR]
                    + ["wa", "wakey", "tgt", "tgc", "ekfon"])
with st.sidebar.expander("🔒 Gizlilik & paylasim"):
    st.markdown("Adetlerin, maliyetlerin, esiklerin ve bildirim anahtarlarin "
                "**sunucuda saklanmaz** — sadece bu sayfanin adresinde (URL) "
                "yasar. Bu yuzden **adres cubugundaki linki kimseyle paylasma**; "
                "paylasirsan portfoyunu ve anahtarlarini da paylasmis olursun.")
    try:
        _temiz = str(st.context.url).split("?")[0]
    except Exception:
        _temiz = None
    if _temiz:
        st.markdown("**Paylasmak icin temiz link:**")
        st.code(_temiz, language=None)
    else:
        st.markdown("Paylasmak icin adresin `?` isaretinden sonrasini silerek "
                    "ver.")
    if st.button("🕶 Kisisel verileri bu ekrandan kaldir",
                 use_container_width=True):
        for _p in KISISEL_PARAMLAR:
            if _p in st.query_params:
                del st.query_params[_p]
        st.rerun()
    st.caption("Kaldir dugmesi sadece bu tarayicidaki adresi temizler; yer "
               "imine kaydettigin kisisel linkin etkilenmez, oradan geri "
               "donebilirsin. Baskasina ekran gosterirken kullan.")
