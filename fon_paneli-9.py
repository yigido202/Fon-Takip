...
with st.sidebar.form("portfoy_formu"):
    ...
    for _kod in FONLAR:
        ...
        _girisler[_kod] = st.number_input(_kod, min_value=0.0, value=_vars,
                                          step=1.0, format="%.2f")
    _kaydet = st.form_submit_button("💾 Kaydet", use_container_width=True)
if _kaydet:
...
st.sidebar.caption("Adetleri yazip Kaydet'e bas. Kaydedilenler adrese "
                   "islenir; sayfayi yer imine eklersen otomatik gelir.")
with st.sidebar.expander("Kutu calismazsa: garantili yontem"):
    st.markdown("Sitenin adresinin sonuna adetleri elle yaz ve Enter'la:\n\n"
                "`...streamlit.app/?TLY=150&THF=200&TP2=1000`\n\n"
                "Sayfa bu adetlerle acilir — hicbir kutuya dokunmadan.")
