...
def _kap_haftalik(kod, tani):
    """KAP JSON API zinciri: kimlik -> bildirim listesi -> ek -> PDF.
    Tum fonlar icin calisir (elle link gerekmez)."""
    _kap_isit()
    oid = kap_fon_oid(kod)
    if not oid:
        tani.append("KAP: fon kimligi bulunamadi (member/filter)")
        return None, None
    b = dict(_KAP_BASLIK)
    b["Accept"] = "application/json"
    b["Referer"] = "https://www.kap.org.tr/tr/bildirim-sorgu"
    try:
        govde = {"fromDate": (simdi_tr() - timedelta(days=45)).strftime("%Y-%m-%d"),
                 "toDate": simdi_tr().strftime("%Y-%m-%d"),
                 "mkkMemberOidList": [oid], "subjectList": []}
        r = requests.post(
            "https://www.kap.org.tr/tr/api/disclosure/members/byCriteria",
            json=govde, headers=b, timeout=15)
        if r.status_code != 200:
        ...
        pdr = [k for k in kayitlar
               if "PORTFOY DAGILIM" in _norm(k.get("subject"))
               or "PORTFOY DAGILIM" in _norm(k.get("summary"))]
        if not pdr:
            tani.append(f"KAP listesinde PDR yok ({len(kayitlar)} bildirim)")
            return None, None
...
AYIKLAYICI_SURUM = 14   # parser her degistiginde artir -> onbellek yenilenir
